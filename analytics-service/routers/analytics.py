...
def overview(
    ...
    recent_orders = [
        o
        for o in orders
        if (_as_datetime(o.get("createdAt")) or since) >= since
    ]

    recent_rentals = [
        r
        for r in rentals
        if (_as_datetime(r.get("createdAt")) or since) >= since
    ]
    ...
    return {
        ...
        "averageOrderValue": (
            round(gross_sales / len(recent_orders), 2)
            if recent_orders
            else 0
        ),
...
def sales_trend(
    ...
    buckets: dict[str, dict[str, float]] = {
        _day_key(since + timedelta(days=i)): {
            "sales": 0.0,
            "rentals": 0.0,
            "orders": 0,
        }
    ...
    for doc in db.collection("rentals").stream():
        ...
        if created and created >= since:
            ...
            if key in buckets:
                buckets[key]["rentals"] += float(
                    rental.get("totalCost", 0)
                )
...
def category_mix(
    ...
    """What's actually selling, by product subcategory."""
    ...
    for doc in db.collection("orders").stream():
        for item in doc.to_dict().get("items", []):
            ...
            revenue_by_sub[sub] += float(
                item.get("lineTotal", 0)
            )

            units_by_sub[sub] += float(
                item.get("qty", 0)
            )
    ...
    rows = [
        ...
        for sub, revenue in sorted(
            revenue_by_sub.items(),
            key=lambda kv: kv[1],
            reverse=True,
        )
    ]
...
def equipment_utilization(
    ...
    """Which shared equipment is over- or under-booked."""

    db = get_db()

    equipment = {
        doc.id: doc.to_dict()
        for doc in db.collection("equipment").stream()
    }
