...
focusableEl?.focus();
  ...
  return (
    <div
      className="fixed inset-0 z-[70] flex items-end justify-center bg-soil/50 dark:bg-husk/50 backdrop-blur-sm sm:items-center sm:p-5"
      role="presentation"
      onMouseDown={(event) => {
    ...
    >
      <AnimatePresence>
        <motion.div
          ...
          } dark:bg-husk/20 dark:shadow-lift`}
        >
          <div className="mb-5 flex items-start justify-between gap-4">
            <h2 className="font-display text-2xl text-soil dark:text-husk">{title}</h2>
            <button
              type="button"
              onClick={onClose}
              aria-label="Close"
              className="grid h-10 w-10 shrink-0 place-items-center rounded-full transition hover:bg-soil/5 dark:hover:bg-husk/5"
            >
              <X size={18} />
            </button>
          </div>
